...
# endif
  implicit none
  logical :: reactive
  ! IJAC to hand to run_odesolver next to jac_native. Resolved once by
  ! set_analytical_jacobian; 1 means the integrator calls jac_native, 0 means it
  ! builds the Jacobian by finite differences and never touches it. Default 0, so
  ! behaviour is unchanged until a mechanism with a Jacobian opts in.
  integer :: IJAC_chem = 0
# if defined(CANTERA)
  type(phase_t) :: gas
# endif
...
contains
  ...
  !----------------------------------------------------------------------------
  ! Resolve IJAC_chem once, after Assign_Mechanism has run. Asking for the
  ! analytical path on a mechanism that has no Jacobian is not an error: it
  ! silently leaves IJAC_chem at 0, which is the pre-existing FD behaviour.
  ! Call this from the input reader, not per step.
  !----------------------------------------------------------------------------
  subroutine set_analytical_jacobian(flag)
    implicit none
    logical, intent(in) :: flag

    IJAC_chem = 0
    if (flag .and. associated(chemistry_jacobian)) IJAC_chem = 1
    if (flag .and. .not. associated(chemistry_jacobian)) &
      write(*,'(A)') ' [JAC] analytical Jacobian requested but the active mechanism has none;'// &
                     ' falling back to finite differences'
  end subroutine set_analytical_jacobian
  ...
  ! the user can decide whether to keep the analytical path on.
  !----------------------------------------------------------------------------
  subroutine validate_analytical_jacobian()
    ...
    if (max_rel_err > 1.d-3) then
      ...
      write(*,'(A)') ' [JAC]          Consider disabling the analytical Jacobian.'
    end if
